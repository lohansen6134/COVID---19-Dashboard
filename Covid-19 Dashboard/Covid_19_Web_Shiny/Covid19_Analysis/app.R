...
ui <- navbarPage("Covid19",
        ...
        tabPanel("tab1",
                 fluidPage(
                     mainPanel(box(plotlyOutput("plot_con", width = 1000)))
                 ),br(),
                 fluidPage(
                     mainPanel(box(plotlyOutput("plot_death", width = 1000)))
                 ),br(),
                 fluidPage(
                     mainPanel(box(plotlyOutput("plot_heal", width = 1000)))
                 )
             )
        # tabPanel("tab2",
        #          fluidPage(
        #              mainPanel(box(plotlyOutput("nation_conf")))
        #          ),br(),
        #          fluidPage(
        #              mainPanel(box(plotlyOutput("nation_dea")))
        #          ),br(),
        #          fluidPage(
        #              mainPanel(box(plotlyOutput("nation_rec")))
        #          )
        #      )
        )
